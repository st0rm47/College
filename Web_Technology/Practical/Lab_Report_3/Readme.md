...
JavaScript is a versatile scripting language mainly used to make web pages interactive. Output in JavaScript can be generated by:

1. **console.log()** – prints messages to the browser console.
2. **alert()** – shows popup alert boxes.
3. **document.write()** – writes directly to the HTML document.
4. **innerHTML** – updates the content of HTML elements dynamically.
...
Regular expressions (regex) are patterns used to match and validate strings. They are often used for input validation, searching, and text manipulation. Here is an example of using regex to validate a name and email in a form.
